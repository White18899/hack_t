...
import '../../public/css/TargetCursor.css';
...
const TargetCursor = ({
  targetSelector = 'button, .btn, .btn-brutal, .btn-claim-stone, .stack-ctrl-btn, a, a.btn-brutal, a.brand-logo, nav.nav-links a, .footer-links a, #open-leader-btn, .nav-toggle-btn, .modal-close, #modal-close-btn, #leader-modal-close-btn, #toast-close, .plan-card, .rule-box, .wanted-poster, .stat-tile, .stone-card, .bounty-card, .prize-poster, .ps-card, .faq-item, .hero-monolith-card, .modal-box, .card-ps-preview-box, .card-arsenal-box, .admin-stat-card, .clock-unit-box, .hero-micro-stat, .hero-tag-pill, .arsenal-pill, .portal-room-pill, .hero-stone-chip, .stamp-badge, .wanted-stamp-red, .wanted-badge-header, .brand-box, .stack-auto-status, .stack-tab-stamp, .countdown-stamp, .hero-clearance-stamp, .badge-domain, .room-badge, .payment-badge, .cursor-target, input, select, textarea',
  spinDuration = 2,
  ...
  useEffect(() => {
    ...
    const scrollHandler = () => {
      ...
      if (!isStillOverTarget) {
        ...
        const { borderWidth, cornerSize, targetPadding } = constants;
    ...
    const enterHandler = e => {
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          ...
          const positions = [
            { x: -cornerSize * 1.4, y: -cornerSize * 1.4 },
            { x: cornerSize * 0.4, y: -cornerSize * 1.4 },
            { x: cornerSize * 0.4, y: cornerSize * 0.4 },
            { x: -cornerSize * 1.4, y: cornerSize * 0.4 }
          ];
